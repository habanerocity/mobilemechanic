...
const ContactForm = (props) => {
	...
	const formSubmissionHandler = (e) => {
		...
		emailjs.sendForm('service_wuh0tvq', 'template_5txpjlx', form.current, 'user_HKI5D9N8lHg0OrMzAbpUM').then(
			(result) => {
			...
			(error) => {
				...
				alert(`${error.text} . Error! Something went wrong, please try to contact me by phone.`);
			}
		);
	...
	const lastNameInputClasses = useDynamicClasses(lastNameHasError, lastNameIsValid);
	const emailInputClasses = useDynamicClasses(emailHasError, emailIsValid);
	const phoneNumInputClasses = useDynamicClasses(phoneNumHasError, phoneNumIsValid);
	...
	const messageInputClasses = useDynamicClasses(messageHasError, messageIsValid);

	return (
		<div className={styles.contact__form}>
			<form ref={form} method="POST" className={styles.contact__form} onSubmit={formSubmissionHandler}>
				...
				<div className={styles.input__container} id={styles.input__containerOne}>
					<div className={`${styles.flex__container} ${styles.contactInfo}`}>
						...
						<input
							...
							id="first-name"
							placeholder={firstNameHasError ? 'Enter a valid name' : 'First Name'}
						...
						<input
							...
							placeholder={lastNameHasError ? 'Enter a valid name' : 'Last Name'}
						...
						<InputMask
							...
							placeholder={phoneNumHasError ? 'Enter phone number' : 'Tel: 555-555–5555'}
					...
					<div className={styles.flex__container} id={styles.input__containerTwo}>
				...
				<div className={styles.message__box}>
					...
					<textarea
						...
						placeholder={messageHasError ? 'How we can help you?' : "Tell us what's wrong with your car"}
						type="text"
				...
				<footer>
					<Button disabled={!formIsValid} className={styles.contact__btn} type="submit">
						Confirm
					</Button>
				</footer>
			</form>
